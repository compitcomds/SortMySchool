<template>
  <div>
    <div v-if="propContent">
      <!-- Timeline -->
      <div
        class="max-w-5xl mx-auto w-full p-5 bg-indigo-150 my-4 rounded-xl shadow-base-200 shadow-xl border-2"
      >
        <template v-for="(blogs, letter) in propContent" :key="letter">
          ...
          <template
            ...
            :key="`listed-blog-${index}`"
          >
            <div
              class="flex gap-1 md:gap-x-3 relative group rounded-lg hover:bg-gray-100"
            >
              <nuxt-link
                :to="`/subject/${blog.subject?.$id || 'others'}/${blog.$id}`"
              ...
              <div class="grow p-2 pb-8">
                <h3
                  class="md:text-lg flex gap-x-1 md:gap-x-1.5 font-semibold text-gray-800"
                >
                  {{ blog.title }}
                </h3>
              </div>
    ...
    <div v-else>
      <ul
        class="flex flex-wrap justify-center items-center gap-5 p-5 max-w-5xl w-full mx-auto"
      >
        <li class="w-full" v-for="i in 3">
          <div
            class="w-full flex flex-col group bg-white rounded-xl overflow-hidden transition min-w-80"
          >
