...
const COLLECTION_SUBJECTS: string = import.meta.env
  .VITE_APPWRITE_COLLECTION_SUBJECT_ID;
const COLLECTION_BLOGS: string = import.meta.env
  .VITE_APPWRITE_COLLECTION_BLOGS_ID;
const COLLECTION_ABOUT: string = import.meta.env
  .VITE_APPWRITE_COLLECTION_ABOUT_ID;
...
export async function getAllSubjects() {
  const subjects = await database.listDocuments(
    DATABASE_ID,
    COLLECTION_SUBJECTS,
...
export async function getBlogDataForSearch() {
  const blogs = await database.listDocuments(DATABASE_ID, COLLECTION_BLOGS, [
    Query.limit(5000),
    Query.select(["$id", "title", "slug", "subject.name"]),
  ]);
  return blogs;
}
...
export async function addViewToBlog(id: string, newViews: number) {
  await database.updateDocument(DATABASE_ID, COLLECTION_BLOGS, id, {
    views: newViews,
  });
  console.log("added a new view to: " + id);
}

export async function submitAboutUsForm(
  name: string,
  collegeName: string,
  yourEmail: string,
  nameOfCaseWithCitation: string,
  additionalComments: string
) {
  const aboutUs = await database.createDocument(
    DATABASE_ID,
    COLLECTION_ABOUT,
    ID.unique(),
    {
